...
2. Sign in with the Microsoft Account which is the Service Administrator of your Azure subscription. 

3.	To create an Azure DevOps organization, provide a unique name, specify the Azure region to host your projects and click **Create**.

    ![](../assets/cdazuresqlvsts\createorg.png)

4.	Once the organization is created, you will be directed to your organization page to create a new project. Set the project name as **AzureSQLDbProject** and click **+ Create project**.

    ![](../assets/cdazuresqlvsts\new_proj.png)

5.	After the project is created, from **Repos** page in the Azure DevOps portal, click **Clone in Visual Studio**. When prompted, click **Allow**. This will start **Visual Studio** with the new project loaded. 

    ![](../assets/cdazuresqlvsts\clone.png)

6. Once the project loads, you will be prompted to clone from the server to your local path.	Change the local path if required and click **Clone**. 
...
13.	Next, click the **Sync** link and then, on the Synchronization page within the Team Explorer window, in the **Outgoing Commits**, click **Push** to push changes to **Azure DevOps**. 

    ![](../assets/cdazuresqlvsts\sync.png)
    ![](../assets/cdazuresqlvsts\push.png)

14.	Switch back to the Azure DevOps interface and refresh the project page.  Under the **Pipelines** section, click on **Build**.

    ![](../assets/cdazuresqlvsts\building.png)

15. Create a new pipeline and select **Use the classic editor** option and click on **Continue**.

    ![](../assets/cdazuresqlvsts\create_pipeline.png)
    ![](../assets/cdazuresqlvsts\classic_editor.png)

16. Now we have to select a template. Click on **Empty Job**, select the **Visual studio** template and click **Add**.

    ![](../assets/cdazuresqlvsts\empty.png)
    ![](../assets/cdazuresqlvsts\visualstudio_template.png)

17.	Accept the default repository source, repository, default branch, enable the **Continuous integration (build whenever this branch is updated)** checkbox, and click **Create**.  

    ![](../assets/cdazuresqlvsts\CI_enabled.png)

18.	On the Build tab of the Build Definitions page, accept the default sequence of build steps and click **Save**. 

    ![](../assets/cdazuresqlvsts\saving.png)

19.	In the Save dialog box, specify a name describing the build definition (we will call it **Default Build**) and click **OK**.  

20.	Now you can queue the build by clicking **Queue** on the Build definitions page. On the **Queue build for Default Build**, accept the default settings and click **Queue**. This will automatically display the Console view on the **Builds** tab in the **Build & Release** section, showing progress of the build process.
...
3.	Switch back to the VSTS interface and, in the **Build & Release** section, click the **Releases** tab and click **+ New definition**.  
    ...
    ![](../assets/cdazuresqlvsts\release(2).png)
