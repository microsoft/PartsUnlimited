...
- Complete the [PartsUnlimited Setup wth Visual Studio 2017](https://microsoft.github.io/PartsUnlimited/pandp/200.1x-PandP-PUsetupwithVS2017.html) task.
...
**Note:** Before starting ensure you have successfully completed the [PartsUnlimited Setup wth Visual Studio 2017](https://microsoft.github.io/PartsUnlimited/pandp/200.1x-PandP-PUsetupwithVS2017.html)
...
11. Click Deploy and if prompted fill in any values the scripts ask for.  This will only happen if you leave a non-auto-generated parameter blank
    ...
    - Errors
        1) If you receive and error saying **Get-ChildItem ... cannot find path**, relating to the json files, this usually occurs when the **Build Action** property of the file  is set to **None**. To resolve this error, in Slutin Explorer** right click the file PartsUnlimitedEnv\Templates\FullenvironmentSetupMerged.json and selct properties, and in the **Properties Pages** set the value for **Build Action** to **Content**.. 
        ...
        2. If you receive errors about Azure resources not available or not identifiable, cannot be found etc it may be related to the parameter values you defined earlier. You should return to those and define them again using the criteria and guidelines outlined earlier.
...
12. The deployment should run successfully in Visual Studio and the infrastructure deployed into Azure as per the screenshots below.
    ...
    **Note:** Some of the resources will be deployed into East US, regardless of what location you specify, so yo may see a mix of locations depending on what you specified. This is because some location values are hard coded into the depployment templates **FullEnvironmentSetupMerged.json**. You can modify these locations if you wish to customise.
...
**Note:** You need to have completed Tasks 1 and 2 earlier to be able to complete Task 3. If you have not completed Tasks 1 and 2 do so now before continuing to Task 3.
