...
3. In the App Service go to **DEPLOYMENT** > **Deployment slots** and note the presence of two deployment slots. the first is for production and was created during deployment, the second is the one we created earlier. Click the second one and then click on **Swap**

    ![PowerShell Tools for Visual Studio](..\assets\manualdeployment-jan2018\VS_deploymentswap3.png)

    Note the option again to **Swap**, and you can change back from **staging** to **dev** for **source** and **destination**. You can also **Delete** the deployment slot but this will not revert the content back to its original state. Thsi will just delete the deployment slots and leave the current state.


4. If you wish you can return to Visual Studio and repeat steps 1 to 6 from Task 4 to create a profile to publish to the URL where the PartsUnlimted has not yet been published to, and verify the website successfully deploys to it.
