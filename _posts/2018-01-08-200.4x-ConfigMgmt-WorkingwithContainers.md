...
2. Start by running a simple windows container with the command prompt, cmd.exe, running in it.
    ...
    - If you receive an error message saying something like *CreateContainer: failure in a Windows system call:...* and *container or virtual hard disk must run in unencrypted and uncompressed disk* this may be due to the disk where the container is trying to tun being compressed or encrypted and you will need to uncompress or unencrypt the drive or run it elsewhere.
