...
9. Stop the container by running the below command 
    ...
    (the name or contianer ID can be obtained from running the **docker ps** command. Also, note that yoyu only need to provide the first two digits of the container ID for the command to run successfully, once it is a unique ID value you do not need to provide the whole ID.)
