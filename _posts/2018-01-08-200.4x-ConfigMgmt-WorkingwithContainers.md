...
2. Start by running a simple windows container with the command prompt, cmd.exe, running in it.
    ...
    A new command window will open which is running inside the container. The context in which the cmd window is running will indicate it is running inside the container as in the screenshot below.
