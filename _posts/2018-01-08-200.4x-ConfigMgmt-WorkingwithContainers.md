...
#### <span style="color: #0000CD;">Create a container image using the docker build file</span>
...
3. Obtain the **IP Address** values for each running container by running the following command, and noting the values listed for each container individually.
    ...
    The IP Adress values will all be different but should be similar to the below
    - 172.19.42.161
    - 172.19.39.17
    - 172.19.42.109

4. Open up a web browser and go to each page i.e. **http://IPAddress-Value**, you shoudl have 

    ![Docker contianer run](../assets/workingwithcontainers-jan2018\dockerrunscale3.png)

    In a few steps we have very quickly spun up three additinal instances of our container web page. This demonstrates how quickly container can be scaled, incompariso to say virtual machines. This is one of the reasons for the popularity and interest in containers.
...
2. Return to the web browser and refresh it, and verify the container web pages are no longer available.
    ...
    **NOTE:** Do **not** delete the image as we will use this in later exercises,
