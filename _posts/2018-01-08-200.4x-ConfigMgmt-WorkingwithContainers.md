...
7. When back on the host run the following command
    ...
    Now you should see a container running with a generated name. Note the name for use in later steps. There are multiple containers running in the screenshot below, you're environment may only have one listed.
...
10. Now run the below commands to view all running containers and then view all containers, including stopped containers.
...
11. Now run the command the below command again
...
13. Verify that the folder and file your created earlier is **not**  present.
...
2. In the running container window run the following commands, to create a folder and .txt file in the container, enter some text into the .txt file, press **Enter** after typing the text, and then exit the container.
