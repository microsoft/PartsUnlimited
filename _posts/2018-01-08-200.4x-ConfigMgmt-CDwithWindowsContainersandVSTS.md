...
10. Configure the **Lock services** task using the below settings. If a setting is not mentioned below leave the default settings.
    ...
    ![](../assets\cdwithcontainersandazdevops-jan2018/buildpipeline8a.png)
...
9. Configure the **first Docker task** to run the container with the following details, where a field is listed leave the default value.
    ...
    - **Docker-compose Files**: < click the ellipsis button at the end of the box and in the artifacts point to `$(System.DefaultWorkingDirectory)/Artifact/docker-compose/releasepipelinecomposefile/docker-compose.yml`. This is a different compose file to that generated by visaual studio earlier. It is one we created ourselves, specifically to run the image as we want on our build agent. You can view the file at <a href="https://github.com/Microsoft/PartsUnlimited/blob/master/Labfiles/Devops200.4x-ConfigMgmtForContainerizedDelivery/M01/Labs/releasepipelinecomposefile/docker-compose.yml" target="_blank"><span style="color: #0066cc;" color="#0066cc">here</span></a> >.
