...
Also, when building containers another factor to take into account is when a container pipeline is run if a new hosted build agent is used the container images need to be downloaded each time onto that build agent, and that will increase your build time. Using a local build agent or a dedicated virtual machine in Azure that you can spin up and down as you need, would mean if the images are already present on the build agent, they would not need to be downloaded again, thus reducing the build time.
