...
4. Open the PartsUnlimited solution in Visual studio, you can do so, by opening **Solution Explorer** and double clicking on **PartsUnlimited.sln**. It may take a few minutes as dependencies will be installed, indicated by an yellow warning sign over some folder i.e. under the **PartsUnlimitedWebsite\Dependencies** folder, wait until that has finished, i.e. when the yellow warning sign is no longer present.
    ...
    **Note**: You do **not** need to a manual build of the application prior to adding Docker support. If you do build the application you may encounter some errors which you'll need to then address. You can see details about addressing potential errors and getting the application to successfully build in Visual Studio in the lab <a href="https://microsoft.github.io/PartsUnlimited/pandp/200.1x-PandP-PUsetupwithVS2017.html" target="_blank"><span style="color: #0066cc;" color="#0066cc"> PartsUnlimited Setup with Visual Studio</span></a>.
...
8. Open the file **PartsUnlimitedWebsite\Dockerfile** and make the following changes. 
    ...
    We also need to use a particular version, as it will run on our build agent which is WS2016 Datacenter with containers, and later versions will not run on this host.


    modify **line 5** to read as below. This is because we will build our application within the container and we require a non nano-server container image to successfully build our application. We also need to use a particular version, as it will run on our build agent which is WS2016 Datacenter with containers, and later versions will not run on this host. See here for <a href="https://docs.microsoft.com/en-us/virtualization/windowscontainers/deploy-containers/version-compatibility" target="_blank"><span style="color: #0066cc;" color="#0066cc"> windows container compatabililty considerations</span></a>
...
It is also possible to automate this agent configuration, and if you look at the Azure Quick Start Templates on github, there are examples of deployment templates for vsts build agents.
...
12. Open a command prompt window as administrator and run the below command to view what packages were installed using chocolatey during deployment. This is the package manager we used during deployment to install some additional software.

    ```powershell
    choco list -localonly
    ```
    **Note**: Ensure that docker is set for Windows containers. If it is set for Linux containers you may receive errors. You can check by clicking the docker icon in the system tray in the bottom right hand side in the virtual machine, and choosing **Switch to Windows Containers...** If the docker icon context menu says **Switch to Linux Containers...**, that means it is set on Windows containers and is fine. If the docker icon context menu says **Switch to Windows Containers...** you should select that option to switch to windows containers, as we will be deploying a windows container to the virtual machine.
...
### Task 9: Create a Build Pipeline
Note: As releases and updates occur frequently in Azure DevOps, there may be some differences in the steps below and what appears in the Azure Devops environment. Where discrepancies occur you should try to adhere to the main configuration points below and accept the default values where possible. 
...
10. Accept the default values for the **Lock services** task and configure the **Copy Files to: $(Build.ArtifactStagingDirectory)** task using the below settings. If a setting is not mentioned below leave the default settings.
    ...
    - **Contents**:   ( ** \ * ) (without the space betweeb the wildchaacters and the forward slash).
...
15. Verify the build runs successfully for both the build an image and push an image tasks.
    ...
    You will notice errors regarding the Loclk service, it is fine to ignore these as it will not affect your build.
...
9. Configure the **first Docker task** to run the container with the following details, where a field is listed leave the default value.
...
10. Configure the second, or **Azure PowerShell script** task, as per the below, again where a field is listed leave the default value.
...
11. Configure the third, the **last Docker task** as per the below, again where a field is listed leave the default value.
...
12. Go to the **Pipeline** and within **Artifacts**, click on the **continuous deployment trigger** icon Triggers tab and enable Continuous Deployment.
