...
1.	In the Azure portal, go to your Automation resource group, then **SHARED RESOURCES** > **Modules** and click on **Update Azure Modules**. if prompted to update all existing Azure modules to the latest version, choose **Yes**.  
    ...
    It will take a few minutes to complete and you will receive a message on the **Modules** pane saying **Azure modules have been udpated** when it is complete.

    ![update automation modules](../assets/azureauto-jan2018\UpdateAzureModules.png)

2.	Click **Browse Gallery**, search for and choose to import the **AzureRM.Network** module. If you are prompted for dependancies, ensure you have updated the existing modules to the latest versions, and try again. 
...
7. Go to **Edit** then the **Test pane** and click **Start**
    ...
    **Note:** You will likely encounter errors when you first test the runbook, this is normal, and you need to work through any errors encounterd one by one using the output in the Test Pane to guide you, i.e. run a test > identify and fix the issue reported, run a test again to verify it is fixed and continue like that in iterations until the script runs successfully.
    ...
    - If you receive errors about a particular resource or parameter value not being avialable in a particular region i.e. **vmsize not being avilaable in wets us** for example, you may  have to change the region you specified to a different one.
