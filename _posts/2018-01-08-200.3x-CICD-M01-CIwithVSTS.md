...
**5.** Select source code location as **Azure Repos Git** and make sure you select correct repository and branch is master. Then click on continue
![](../assets/civsts-jan2018/shot5a.png)

**6.** Select the **Empty job** as the template.
 ...
 **9.** Still in the **Add tasks** dialog, select the **utility** page and add a **Copy Files to** task.
...
**11.** On the **PowerShell Script** task, click on the blue **rename** pencil icon and change the name of the step to **dotnet restore, build, test and publish** and click **OK**

**12.** Select **File Path** for the **Type** property, enter **"build.ps1"** for the **Script filename** property and **$(BuildConfiguration) $(Build.SourcesDirectory) $(build.stagingDirectory)** for the **Arguments** property.
...
**13.** On the **Publish Test Results** task, change the **Test Result Format** to **VSTest** and the **Test Results File** to **\*\*\testresults.xml**.

![](../assets/civsts-jan2018/shot10d.png)


**14.** On the **Copy Files To** task, change the **Source Folder** to **$(build.sourcesdirectory)** and the **Target Folder** to **$(build.artifactstagingdirectory)**.

![](../assets/civsts-jan2018/shot12.png)


**15.** On the **Publish Artifact** task, change the **Path to publish** to **$(build.artifactstagingdirectory)** and the **Artifact name** to **drop**.

![](../assets/civsts-jan2018/shot13.png)



**16** Select the **Variables** tab and a new variable that will be used by the build.ps1 PowerShell script; **BuildConfiguration** with a value of **release**.

![](../assets/civsts-jan2018/shot14.png)


**17.** Click on the **Triggers** tab and verify that the **Enable continuous integration (CI)** option is enabled for your project to build the solution every time a change is checked in. Also make sure the filter includes the appropriate branch, in this case **master*, also and that **Batch changes while a build is in progress** checkbox is unchecked.
...
**18.** Click **Save & queue** and select the **Save** option.
...
![](../assets/civsts-jan2018/shot20.png)
