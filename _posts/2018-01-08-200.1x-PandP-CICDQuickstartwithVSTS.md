...
### Task 4: Import Continuous Integration Build Definition into VSTS
...
12. Up until now, when all errors have rectified, the **Save** option was not available, it was greyed out. However, now that there are no errors present, all that is left to do is to click **Save and Queue** for the release. 
...
1. Navigate to the **Build** tab. Click on the ellipsis **(...)** button next to the build definition you would like to export and select "Export". This will trigger a download of the build definition in JSON format. Place this file in the `templates\build definitions` directory of your local repository.
