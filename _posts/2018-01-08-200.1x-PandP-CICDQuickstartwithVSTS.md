...
11. Under the **Azure App Service Deploy....** task, you will need to  update the **Azure subscription** , again to the **service connection** value you created earlier in **Task 3**, and the **Slot**. For Slot you will need to enter the value **Dev**. This will default to the value `production`, but it needs to be changed to `dev` in the `dev` task. 
