...
### Task 2: Apply a template using Powershell that removes all resources in a resource group (Portal).
...
6.	From Windows PowerShell, deploy the template to the **rg1** resource group, or whatever your resource group is called, by running the command below, 
    ...
    ```powershell
    New-AzureRmResourceGroupDeployment -ResourceGroupName 'rg1' -Mode Complete -TemplateFile $template -Force
    ```
...
6.	Back on the **Custom deployment** blade, use the following settings: 
    ...
    - virtualmachines_vm1_adminpassword = **Pa55w.rd1234**
    - virtualmachines_vm1_adminpassword = **Pa55w.rd1234**
