...
9. Click on the **Azure Resource Group Deployment** task and configure it as follows:
	...
	- Override template parameters = Copy and paste the below in a single line (shown split here for convenience). It would be wise to copy into a text editor, such as notepad first to ensure formatting is as expected and to remove any potential character corruption, then copy from that editor into VSTS.
...
10. Click on the ellipsis (...) button next to the Environment and select "Configure variables..."
