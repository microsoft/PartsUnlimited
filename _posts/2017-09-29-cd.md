...
6. Once the deployment completes, you can check that the site was in fact deployed successfully by navigating to the site url.
	...
	> Since you deployed to the dev slot, you will need to navigate to **http://{siteName}-dev.azurewebsites.net** where siteName is the name of your Web App in Azure. You can obtain the URL from opening the **Dev** Web App in Azure and copying the URL from within the **Overview** pane.

	![](/PartsUnlimited/assets/cd2/DeployedtoDevIE.png)

	> You can also go to the staging and production sites, i.e. **http://{siteName}-staging.azurewebsites.net** and **http://{siteName}.azurewebsites.net**. The pages will resolve but the app has not been deployed to them yet, so is not present.
...
4. A new Environment is created. Enter **Staging** for the **Environment name**. You can then click the **X** in the right hand corber of the Environment dialogue to close it.

	![](/PartsUnlimited/assets/cd2/Stagingconfig1.png)

5. Click **Tasks* and select **Staging**
...
9. Click **Pipeline** and on the **Staging** environment click the **Pre-deployment conditions** icon present at the start of the **Staging** environment box

	![](/PartsUnlimited/assets/cd2/StagingPredeployConds1.png)


9. Configure the **Pre-deployment conditions** pane **Triggers** section as below, to trigger a staging deployment after the dev deployment is completed. 
...
11. Close the **Pre-deployment conditions** dialogue by clicking the **X** in the top right hand corner of the dialogue and **Save** the release definition

12. Click the **Post-deployment conditions** icon at the end of the **Staging** environment box

	![](/PartsUnlimited/assets/cd2/StagingPostdeployConds1.png)

13. Configure the **Post-deployment conditions** pane **Post-deployment approvers** section as below, then save the release defintion and close the dialogue when finished configuring it.
...
16. Now verify the **Post-deployment conditions are configured as below
